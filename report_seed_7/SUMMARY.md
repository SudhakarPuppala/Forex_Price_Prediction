...
| Hybrid_CNN_LSTM_Transformer | 0.03393 | 0.04458 | 208.1 | 0.5874 | 0.5000 |
| Vanilla_LSTM | 0.04200 | 0.05663 | 339.5 | 0.4951 | 0.4962 |
...
- Lowest overall MAE: Hybrid_CNN_LSTM_Transformer (0.03393).
- Highest directional accuracy: XGBoost_standalone (0.5882).
- Caution: the proposed Hybrid model does not outperform XGBoost_standalone on this run. On data without a strong, real cross-modal signal, extra model capacity tends to fit noise rather than add predictive power — see the README for guidance on validating the architecture against data with a known injected signal, and on real market data once available.
- Vanilla_LSTM's directional accuracy (0.4951) is close to the 0.5 random-guess baseline — treat any directional edge from this run as inconclusive rather than a confirmed skill.
